...
const ChatPage = () => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f0c29, #302b63, #24243e)', display: 'flex', fontFamily: 'Segoe UI, sans-serif', color: '#fff' }}>
      {/* Sidebar - Courses */}
      <div style={{ width: '280px', background: 'rgba(0,0,0,0.3)', borderRight: '1px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1rem' }}>
          {courses.length === 0 ? (
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.9rem', textAlign: 'center', marginTop: '2rem' }}>No courses available</p>
          ) : (
            courses.map(course => (
              <div key={course._id} onClick={() => setSelectedCourse(course)}
                style={{ padding: '1rem', borderRadius: '10px', marginBottom: '0.5rem', cursor: 'pointer', background: selectedCourse?._id === course._id ? 'linear-gradient(135deg, rgba(102,126,234,0.3), rgba(118,75,162,0.3))' : 'rgba(255,255,255,0.05)', border: `1px solid ${selectedCourse?._id === course._id ? 'rgba(102,126,234,0.5)' : 'rgba(255,255,255,0.08)'}`, transition: 'all 0.2s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #667eea, #764ba2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', fontWeight: 700 }}>{course.name?.charAt(0)}</div>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>{course.name}</div>
                    <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.75rem' }}>{course.code || 'Course Chat'}</div>
                  </div>
                </div>
              </div>
            ))
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        {!selectedCourse ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '1rem' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)' }}>Connect with your classmates and teachers</p>
          </div>
        ) : (
          <>
            {/* Chat Header */}
            <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(135deg, #667eea, #764ba2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', fontWeight: 700 }}>{selectedCourse.name?.charAt(0)}</div>
              <div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
              {loading ? (
                <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.5)', marginTop: '2rem' }}>Loading messages...</div>
              ) : messages.length === 0 ? (
            ...
            <form onSubmit={sendMessage} style={{ padding: '1rem 1.5rem', borderTop: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.2)', display: 'flex', gap: '0.8rem' }}>
              <input value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder={`Message ${selectedCourse.name}...`}
                style={{ flex: 1, padding: '0.8rem 1.2rem', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '25px', color: '#fff', fontSize: '0.9rem', outline: 'none' }} />
              <button type="submit" disabled={!newMessage.trim()}
                style={{ padding: '0.8rem 1.5rem', background: newMessage.trim() ? 'linear-gradient(135deg, #667eea, #764ba2)' : 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '25px', color: '#fff', cursor: newMessage.trim() ? 'pointer' : 'default', fontWeight: 600, transition: 'all 0.2s' }}>Send</button>
            </form>
          </>
        )}
      </div>
